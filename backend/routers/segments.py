...
from ..models import Device, Segment, User
...
def delete_segment(
    ...
    if not segment:
        raise HTTPException(status_code=404, detail="Segment not found")
    # Explicitly null out devices assigned to this segment.
    # SQLite only enforces ON DELETE SET NULL when PRAGMA foreign_keys=ON,
    # which is set in database.py — but we do this explicitly to be safe.
    db.query(Device).filter(Device.segment_id == segment_id).update({"segment_id": None})
    db.delete(segment)
    db.commit()
