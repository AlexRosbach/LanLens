...
test('settings groups routine jobs, lifecycle, and network discovery separately', async ({ page }, testInfo) => {
  ...
  await page.getByRole('heading', { name: 'Scan Nodes' }).scrollIntoViewIfNeeded()
  await expect(page.getByText('Optional scanners per VLAN or site.')).toBeVisible()
  await expect(page.getByPlaceholder('Site, e.g. London')).toBeVisible()
  await expect(page.getByText('No Scan Nodes configured yet.')).toBeVisible()
  await page.screenshot({ path: testInfo.outputPath('settings-scan-nodes-english.png'), fullPage: false })
