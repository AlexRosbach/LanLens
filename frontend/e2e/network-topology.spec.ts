...
test('network topology visualizes inventory and SNMP relationships', async ({ page }, testInfo) => {
  ...
  const nasNode = page.locator('[data-testid="topology-node"][data-node-id="4"]')
  const nasEdge = page.locator('[data-testid="topology-edge"][data-source-id="2"][data-target-id="4"]')
  const nasBoxBefore = await nasNode.boundingBox()
  const nasEdgeBefore = await nasEdge.getAttribute('d')
  if (!nasBoxBefore || !nasEdgeBefore) throw new Error('NAS topology node did not render')
  await page.mouse.move(nasBoxBefore.x + nasBoxBefore.width / 2, nasBoxBefore.y + nasBoxBefore.height / 2)
  await page.mouse.down()
  await page.mouse.move(nasBoxBefore.x + nasBoxBefore.width / 2 + 120, nasBoxBefore.y + nasBoxBefore.height / 2 + 55)
  await page.mouse.up()
  const nasBoxAfter = await nasNode.boundingBox()
  const nasEdgeAfter = await nasEdge.getAttribute('d')
  if (!nasBoxAfter || !nasEdgeAfter) throw new Error('NAS topology node disappeared after dragging')
  expect(Math.abs(nasBoxAfter.x - nasBoxBefore.x)).toBeGreaterThan(60)
  expect(Math.abs(nasBoxAfter.y - nasBoxBefore.y)).toBeGreaterThan(10)
  expect(nasEdgeAfter).not.toBe(nasEdgeBefore)
  await page.getByRole('button', { name: 'Reset topology layout' }).click()
